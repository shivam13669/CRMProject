...
const DOCUMENT_TYPES: CustomerDocument['type'][] = ['Aadhaar', 'Aadhar', 'PAN', 'Other'];
const normalizeDocumentType = (value: unknown): CustomerDocument['type'] => {
  const normalized = String(value ?? '').trim();
  const match = DOCUMENT_TYPES.find(type => type.toLowerCase() === normalized.toLowerCase());
  return match ?? 'Other';
};
  ...
  try {
    ...
    try {
      ...
      if (!adminCols.includes('name')) {
        try { db.run(`ALTER TABLE admins ADD COLUMN name TEXT NOT NULL DEFAULT 'Admin'`); } catch {}
      }
      ...
      if (!adminCols.includes('status')) {
        try { db.run(`ALTER TABLE admins ADD COLUMN status TEXT CHECK(status IN ('active','suspended')) NOT NULL DEFAULT 'active'`); } catch {}
      }
    } catch {}
...
async function createDefaultAdmin() {
  try {
    ...
    if (result.count === 0) {
      ...
      db.run(`
        INSERT INTO admins (username, name, role, status, password_hash)
        VALUES (?, ?, ?, ?, ?)
      `, ['admin@gmail.com', 'Admin', 'admin', 'active', hashedPassword]);
    ...
    try {
      ...
      if (!hasCentral) {
        ...
        db.run(`
          INSERT INTO admins (username, name, role, status, password_hash)
          VALUES (?, ?, ?, ?, ?)
        `, ['admincr@gmail.com', 'Central Admin', 'central', 'active', centralHash]);
...
export const authDB = {
  ...
  async verifyAdmin(username: string, password: string): Promise<{ success: boolean; admin?: any }> {
    try {
      ...
      if (isValid) {
        const status = String((admin as any).status || 'active');
        if (status !== 'active') {
          return { success: false, suspended: status === 'suspended' } as any;
        }
  ...
  async getAllAdmins(): Promise<any[]> {
    try {
      ...
      const stmt = db.prepare('SELECT id, username, name, role, status, created_at, updated_at FROM admins ORDER BY created_at DESC');
      const results = [];
      while (stmt.step()) {
        const row = stmt.getAsObject();
        results.push({
  ...
  async getAdminById(id: string): Promise<any | null> {
    try {
      await ensureInitialized();
      const stmt = db.prepare('SELECT id, username, name, role, status FROM admins WHERE id = ?');
      stmt.bind([id]);
      if (stmt.step()) {
        const row = stmt.getAsObject();
        return { id: String(row.id), username: String(row.username), name: String(row.name), role: String(row.role), status: String(row.status) };
      }
      return null;
    } catch (e) {
  ...
  async setAdminStatus(id: string, status: 'active' | 'suspended'): Promise<boolean> {
    try {
      await ensureInitialized();
      db.run('UPDATE admins SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [status, id]);
      await saveDatabase();
      return true;
    } catch (e) {
  ...
  async updateAdmin(id: string, fields: { name?: string; username?: string; password?: string }): Promise<boolean> {
    try {
      await ensureInitialized();
      const sets: string[] = [];
      const params: any[] = [];
      if (fields.name != null) { sets.push('name = ?'); params.push(fields.name); }
      if (fields.username != null) { sets.push('username = ?'); params.push(fields.username); }
      ...
      const sql = `UPDATE admins SET ${sets.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
      params.push(id);
      db.run(sql, params);
      await saveDatabase();
      return true;
